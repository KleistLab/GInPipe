...
readData <- function(datadir, Nsim, Nbins) {
  ...
  for(i in 1:Nsim) {
    ...
    trueN_simuData.table = rbind(trueN_simuData.table, data.frame(t=input.table_NS$t, trueN=input.table_NS$trueN, meanBinDate=input.table_NS$meanBinDate, simu=i))
    ...
    #Splines for true N
    trueSpline_table <- addSplineValuesForTrueN(input.table_NS, splinePred_simu_NS)
    splinePred_trueN <- data.frame(t=trueSpline_table$t, value=trueSpline_table$value_trueN, 
                                   lower=trueSpline_table$value_trueN_lower,
                                   upper=trueSpline_table$value_trueN_upper)
  ...
  #in the simulation we took the time times 7, but thats a too big span...
  all_simuData.table$t <- all_simuData.table$t/7
  trueN_simuData.table$t <- trueN_simuData.table$t/7
  all_splinePredictions.table$t <- all_splinePredictions.table$t/7
  trueN_splinePrediction.table$t <- trueN_splinePrediction.table$t/7
...
readData_intros <- function(datadir, Nsim, Nbins, num_intros) {
  ...
  for(i in 1:Nsim) {
    ...
    for(ni in num_intros) {
      ...
      all_simuData.table = rbind(all_simuData.table, 
                                 data.frame(input.table_NS_intro, firstOrigins=input.table_NS_intro$origins, totalOrigins=sum(input.table_NS_intro$origins), sample="NS",simu=i, subsample=1, intro=ni), 
                                 data.frame(input.table_WSMB_intro, totalOrigins=sum(input.table_WSMB_intro$firstOrigins), sample="WSMB",simu=i, subsample=input.table_WSMB_intro$sampled_N/input.table_WSMB_intro$trueN, intro=ni))
      ...
      #Splines for true N
      trueSpline_table <- addSplineValuesForTrueN(input.table_NS_intro, splinePred_simu_NS_intro)
      splinePred_trueN_intro <- data.frame(t=trueSpline_table$t, value=trueSpline_table$value_trueN, 
                                     lower=trueSpline_table$value_trueN_lower,
                                     upper=trueSpline_table$value_trueN_upper)
      ...
      trueN_simuData.table = rbind(trueN_simuData.table, 
                                   ...
                                   data.frame(t=input.table_NS_intro$t, trueN=input.table_NS_intro$trueN, meanBinDate=input.table_NS_intro$meanBinDate, simu=i, intro=ni))
